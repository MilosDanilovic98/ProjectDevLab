// api key
const api_key = "&apiKey=e3fb4f5299d44066b025296877ccc8e4";
...
function displayList(items, wrapepr, rows_per_page, page) {
  ...
  paginatedNews.forEach((news) => {
    ...
    news.urlToImage == null || news.urlToImage == ""
      ? (cover_image = "../images/list/no_img.png")
      : (cover_image = news.urlToImage);
    news.description != null
      ? (description =
          JSON.stringify(news.description).substring(0, 250) + "...")
      : (description = " Click on image to read full news ");
...
form.addEventListener("submit", (e) => {
  ...
  if (selection.value == "popularity") {
    sortBy = "popularity";
  } else if (selection.value == "relevancy") sortBy = "relevancy";
...
categories.addEventListener("click", (e) => {
  ...
  if (e.target.value.length > 0) {
    ...
    document.querySelector(
      `#${e.target.value}`
    ).parentElement.style.backgroundColor = "#44FFEE";
  }
});
